...
contract TokenizeMintTest is Test {
    ...
    function testRevertRequestAlreadyFulfilled() public {
        vm.selectFork(ethfork);
        // create wallet
        address user1 = vm.addr(1);
        uint256 index = michiWalletNFT.currentIndex();
        ...
        // try to create exact same request again
        vm.expectRevert(abi.encodeWithSelector(MichiPointsMinter.RequestAlreadyFulfilled.selector, ethfork, requestId));
        michiPointsMinter.mintTokenizedPoints(requestor, a, b, ethfork, requestId);
    }
